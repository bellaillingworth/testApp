...
export default function ChecklistScreen() {
  ...
  const toggleTask = async (month: string, taskId: string) => {
    setTasksByMonth(prev => {
      const newTasksByMonth = { ...prev };
      newTasksByMonth[month] = newTasksByMonth[month].map(task =>
        task.id === taskId ? { ...task, done: !task.done } : task
      );
      
      // Update completed count
      const allTasks = Object.values(newTasksByMonth).flat();
      setCompletedCount(allTasks.filter(task => task.done).length);
      
      // Save progress after updating
      saveProgress(currentGrade, newTasksByMonth);
      
      return newTasksByMonth;
    });
  };
